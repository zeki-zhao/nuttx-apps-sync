...
static int fb_fd = -1;
struct fb_overlayinfo_s chanegefb;
/****************************************************************************
 * Public Functions
 ****************************************************************************/
...
int main(int argc, FAR char *argv[])
{
    ...
    *( uint32_t*) (SDRAM_BANK_ADDR+20 ) = (uint32_t)0xCCCCCCCC;
    ...
    if(fb_fd > 0){
        printf("open fb0 success, fd is %d\n",fb_fd);
    }


    ioctl(fb_fd,FBIOGET_VIDEOINFO,0); ///* Get color plane info */
    ioctl(fb_fd,FBIO_SELECT_OVERLAY,0); ///* Get color plane info */
    while(1)
    {
        ioctl(fb_fd,FBIOSET_COLOR,0); ///* Get color plane info */
...
int main(int argc, FAR char *argv[])
{
  ...
  //   char TempStr[100];
  //   int fb_fd = open("/dev/kmsg", O_WRONLY);
  //   // int fb_fd2 = open("/dev/console", O_WRONLY);
  //   // if(fb_fd > 0){
  //   // printf("open console success, fd is %d\n",fb_fd);
  //   // }
  //   // read(fb_fd, TempStr, 100);
