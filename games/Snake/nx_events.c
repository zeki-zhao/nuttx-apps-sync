...
const struct nx_callback_s g_snake_nxcb =
{
  nxeg_redraw,     /* redraw */
...
#ifndef CONFIG_EXAMPLES_NX_RAWWINDOWS
const struct nx_callback_s g_snake_tbcb =
{
                          ...
                          FAR void *arg)
{
  ...
  if (!b_snake_haveresolution)
    {
      /* Save the window limits (these should be the same for all places and all windows */

      g_snake_xres = bounds->pt2.x;
      g_snake_yres = bounds->pt2.y;

      b_snake_haveresolution = true;
      sem_post(&g_snake_semevent);
                          ...
                          bool more, FAR void *arg)
{
  ...
  printf("nxeg_tbredraw%d: hwnd=%p rect={(%d,%d),(%d,%d)} more=%s\n",
         ...
         rect->pt1.x, rect->pt1.y, rect->pt2.x, rect->pt2.y,
         more ? "true" : "false");
  nxeg_filltoolbar(hwnd, rect, g_snake_tbcolor);
}
#endif
  ...
  for (;;)
    {
      ...
      if (!g_snake_connected)
        {
          g_snake_connected = true;
          sem_post(&g_snake_semevent);
